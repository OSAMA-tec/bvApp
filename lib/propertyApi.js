...
const propertyAPI = {
    ...
    // Tokenize property
    tokenizeProperty: async (id, tokenData, token) => {
        try {
            console.log(token)
            const { data } = await api.post(`/properties/${id}/tokenize`, tokenData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                    'Accept': 'application/json'
    ...
    // Transfer property
    transferProperty: async (id, transferData, token) => {
        try {
            const { data } = await api.post(`/properties/${id}/transfer`, transferData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                    'Accept': 'application/json'
